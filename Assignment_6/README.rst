...
This will create both a top.bin which can be flashed, as well as top.jlink which can be used with J-Link, in the project root dir.

Build M4
From the project root dir, execute:

make -C m4/GCC_Project
This will generate a m4.bin in m4/GCC_Project/output/bin which can be flashed.
...
#*******************
[0] >
Enter the GPIO Controller submenu using gpioctlr and then type help in the gpioctlr submenu for commands.

[0] > gpioctlr
[1] gpioctlr > help
help-path: gpioctlr
setout     - set gpio as output with value specifiedHow ToHow To
======
...
val = 1
[1] gpioctlr >
To read GPIO value, we need to ensure that this IO has been set as input first using setin IO_NUMBER and then we can read the value using getval IO_NUMBER
...
Control GPIO as output or input be setting 1 or 0 respectively to the appropriate bit in REG_ADDR_GPIO_OE (0x40024008)
If set as output, set the appropriate bit as 1 or 0 for High/Low in REG_ADDR_GPIO_OUT (0x40024004)
If set as input read the value at appropriate bit from REG_ADDR_GPIO_IN (0x40024000)
